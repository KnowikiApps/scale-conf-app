...
RUN --mount=type=secret,id=qt_acct_email \
    ...
    ./qt-unified-linux-x64-4.6.1-online.run --root /Qt --accept-licenses --confirm-command --accept-messages install qt.qt6.673.android --email $(cat /run/secrets/qt_acct_email) --pw $(cat /run/secrets/qt_acct_pw) --accept-obligations
